...
Clothes = function(csv) {
  ...
  return {
    own: false,
    name: csv[0]+(w_adj[typeToNum(csv[1].split('-')[0])+'.'+csv[2]]?'':'*'),
    type: theType,
    id: csv[2],
    longid: clotonum(csv[1],csv[2]),
    stars: csv[3],
    simple: realRating(csv[5], csv[4], theType, csv[2], 0),
    cute: realRating(csv[9], csv[8], theType, csv[2], 4),
    active: realRating(csv[7], csv[6], theType, csv[2], 2),
    pure: realRating(csv[11], csv[10], theType, csv[2], 6),
    cool: realRating(csv[12], csv[13], theType, csv[2], 8),
    tags: csv[14].split('/'),
...
function typeToNum(mainTypeName){
  switch(mainTypeName) {
    case '发型': return '1';
    case '连衣裙': return '2';
    case '外套': return '5';
...
function realRating(a, b, type, id, pos) {
  real = a ? a : b;
  symbol = a ? 1 : -1;
  cateId = typeToNum(type.mainType);
  realPos = parseInt(pos + (a ? 1 : 0));
  realRatingScore = w_adj[cateId+'.'+id] ? w_adj[cateId+'.'+id][realPos]*15 : type.score[real];
  score = symbol * realRatingScore;
...
//full.html - flist handling

function numtolongid(numb){
	if (numb>880000 && numb<890000) return 'A'+numb.toString().substr(2);
...
var Flist = function() {
	var ret = Flist;
	if(flistWhite) {
		for (var theme in flistWhite){
			if (!ret[theme]) ret[theme] = {};
			if (!ret[theme]["type"]) ret[theme]["type"] = [];
			for (var i in flistWhite[theme]){
				for (var j in clothes){
					if (clothes[j].longid==numtolongid(flistWhite[theme][i])){
						var cloTarget = clothes[j];
						var cloLongid = clothes[j].longid;
						ret[theme][cloLongid]='A';
						checkPush(cloTarget.type.type, ret[theme]["type"]);
						break;
					}
				}
			}
			for (var i in repelCates){
				for (var j in repelCates[i]){
					if ($.inArray(repelCates[i][j], ret[theme]["type"])>=0){
						for (var k in repelCates[i]) checkPush(repelCates[i][k], ret[theme]["type"]);
					}
				}
			}
		}
	...
	if(flistBlack) {
		for (var theme in flistBlack){
			if (!ret[theme]) ret[theme] = {};
			for (var i in flistBlack[theme]){
				var cloTarget=numtolongid(flistBlack[theme][i]);
				ret[theme][cloTarget]='F';
			}
		}
	}
	if(flistWhiteRange) {
		for (var theme in flistWhiteRange){
			if (!ret[theme]) ret[theme] = {};
			if (!ret[theme]["range"]) ret[theme]["range"] = [];
			for (var i in flistWhiteRange[theme]){
				ret[theme]["range"].push(flistWhiteRange[theme][i]);
			}
		}
	}
	return ret;
